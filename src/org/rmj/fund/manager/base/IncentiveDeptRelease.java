...
public class IncentiveDeptRelease {
    ...
    private ArrayList<DeptIncentive> p_oDetail;
    ...
    public DeptIncentive getDetail(int fnRow){
...
//            }
            if (p_oIncentive.SearchTransaction(loRS.getString("sTransNox"),true)){
                ...
                DeptIncentive detail = p_oIncentive;
                
//                detail.setMaster("sTransNox", p_oIncentive.getMaster("sTransNox"));
//                detail.setMaster("dTransact", p_oIncentive.getMaster("dTransact"));
//                detail.setMaster("sDeptIDxx", p_oIncentive.getMaster("sDeptIDxx"));
//                detail.setMaster("sInctveCD", p_oIncentive.getMaster("sInctveCD"));
//                detail.setMaster("sRemarksx", p_oIncentive.getMaster("sRemarksx"));
//                detail.setMaster("dEffctive", p_oIncentive.getMaster("dEffctive"));
//                detail.setMaster("cTranStat", p_oIncentive.getMaster("cTranStat"));
//                detail.setMaster("sApproved", p_oIncentive.getMaster("sApproved"));
//                detail.setMaster("dApproved", p_oIncentive.getMaster("dApproved"));
//                detail.setMaster("sModified", p_oIncentive.getMaster("sModified"));
//                detail.setMaster("dModified", p_oIncentive.getMaster("dModified"));
//                detail.setMaster("xBranchNm", p_oIncentive.getMaster("xBranchNm"));
//                detail.setMaster("xDeptName", p_oIncentive.getMaster("xDeptName"));
//                detail.setMaster("xInctvNme", p_oIncentive.getMaster("xInctvNme"));
//                  detail = p_oIncentive;
                System.out.println(detail.getMaster("sTransNox"));
                p_oDetail.add(detail);
    ...
    public boolean SaveTransaction() throws SQLException{
        ...
        for (lnCtr = 0; lnCtr <= p_oTag.size()-1; lnCtr++){
            if (p_oTag.get(lnCtr).equals("1")){
                ...
                if (p_oApp.executeQuery(lsSQL, "Department_Incentive_Master", p_sBranchCd, ((String) p_oDetail.get(lnCtr).getMaster("sTransNox")).substring(0, 4)) <= 0){
                ...
                for (lnCtr2 = 1; lnCtr2 <= p_oDetail.get(lnCtr).getItemCount(); lnCtr2++){
                    lnTranTotl += (double) p_oDetail.get(lnCtr).getDetail(lnCtr2, "sNewAmtxx");
                }
        ...
        p_oMaster.updateObject("sNewAmtxx", lnTranTotl);
...
//            System.out.println("Column index: " + (lnCtr) + " --> Label: " + p_oMaster.getMetaData().getColumnLabel(lnCtr));
             //System.out.println(p_oDetail.get(lnCtr).getMaster("sTransNox"));
        }
