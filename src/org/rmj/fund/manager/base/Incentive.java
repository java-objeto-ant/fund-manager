...
public class Incentive {
    ...
    private final String ALLOWED_VERIFICATION_ON_APPROVAL = "app.allowed.verifiction.on.approval";
    ...
    public boolean NewTransaction() throws SQLException {
        ...
        if (System.getProperty(DEBUG_MODE).equals("0")) {
            System.out.println(p_oApp.getEmployeeLevel());
        ...
        if (System.getProperty(DEBUG_MODE).equals("0")) {
            if (!System.getProperty(ALLOWED_VERIFICATION_ON_APPROVAL).contains(p_oApp.getEmployeeNo())) {
                if (Integer.valueOf(p_oApp.getEmployeeLevel()) < 1) {
                    p_sMessage = "Your employee level is not authorized to use this transaction.";
                    return false;
                } else if (!("036" + "»" + "015" + "»" + MIS + "»" + AUDITOR + "»" + FINANCE + COLLECTION).contains(p_oApp.getDepartment())) {
                    p_sMessage = "Your employee level is not authorized to use this transaction.";
                    return false;
                }
                if (p_oApp.getUserLevel() < UserRight.SUPERVISOR) {
...
//        
    ...
    public boolean searchIncentive(String fsValue, boolean fbByCode) throws SQLException {
        ...
        if (fbByCode) {
            ...
            for (lnCtr = 1; lnCtr <= lnRow; lnCtr++) {
                if (fsValue.equals((String) getIncentiveInfo(lnCtr, "xInctvNme"))) {
                    return true;
                }
            }
        }
    ...
    public boolean searchDepartment(String fsValue, boolean fbByCode) throws SQLException {
        ...
        if (fbByCode) {
            if (fsValue.equals((String) getMaster("sDeptIDxx"))) {
                return true;
            } else if (fsValue.equals((String) getMaster("xDeptName"))) {
                return true;
            }
        }
...
//                System.out.println("sDeptName = " +(String) loJSON.get("sDeptName"));
                ...
                if (p_oListener != null) {
                    p_oListener.MasterRetreive(17, getMaster("xDeptName"));
                }
    ...
    public boolean searchBranch(String fsValue, boolean fbByCode) throws SQLException {
        ...
        if (p_bWithUI) {
            ...
            if (loJSON != null) {
                ...
                if (p_oListener != null) {
                    p_oListener.MasterRetreive(16, getMaster(16));
                }
                return true;
            } else {
                p_sMessage = "No record selected.";
...
//        if (p_oListener != null) p_oListener.DetailRetreive(0, 0, "");
    ...
    private boolean addIncentive(String fsCode, String fsDescript, String fcByPercnt) throws SQLException {
        ...
        for (lnCtr = 1; lnCtr <= lnRow; lnCtr++) {
            if (fsCode.equals((String) getIncentiveInfo(lnCtr, "sInctveCD"))) {
                return true;
            }
        }
...
//            System.out.println("department  = " + p_oApp.getDepartment());
    ...
    private void loadConfig() {
        ...
        System.setProperty(ALLOWED_VERIFICATION_ON_APPROVAL,
                "M00123000028»"
                + "M00119000199»"
                + "M00124001135»"
                + "M00124000848»"
